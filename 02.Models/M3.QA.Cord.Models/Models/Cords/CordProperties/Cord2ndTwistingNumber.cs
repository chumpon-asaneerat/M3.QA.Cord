...
namespace M3.QA.Models
{
    ...
    /// </summary>
    public class Cord2ndTwistingNumber : NInpc
    {
        #region Constructor
        ...
        /// Constructor.
        /// </summary>
        public Cord2ndTwistingNumber() : base()
        {
        ...
        /// <returns></returns>
        internal static List<Cord2ndTwistingNumber> Create(CordSampleTestData value,
            Utils.M_GetPropertyTotalNByItem totalN)
        {
            List<Cord2ndTwistingNumber> results = new List<Cord2ndTwistingNumber>();
            if (null == value)
                return results;

            // For 2nd Twisting Number Proepty No = 8
            int noOfSample = (null != totalN) ? totalN.NoSample : 0;
            int alllowSP = (value.TotalSP.HasValue) ? value.TotalSP.Value : 0;

            // 2nd Twisting Number Proepty No = 8
            var spec = value.Specs.FindByPropertyNo(8);
            ...
            while (i <= iMaxLimitSP)
            {
                ...
                var inst = new Cord2ndTwistingNumber()
                {
                    LotNo = value.LotNo,
                    PropertyNo = 8, // 2nd Twisting Number = 8
                    SPNo = SP,
                    NeedSP = true,
                    Spec = spec,
            ...
            // For 2nd Twisting Number Proepty No = 8
            var existItems = (value.MasterId.HasValue) ? GetsByLotNo(value.LotNo, 8).Value() : null;
            ...
            if (null != results)
            {
                //foreach (var item in results) item.CalculateFormulaFromItem(); // Check spec to update UI
            }
        ...
        public static void Clone(Cord2ndTwistingNumber src, Cord2ndTwistingNumber dst)
        {
        ...
        /// <summary>
        /// Gets Cord2ndTwistingNumber by Lot No.
        /// </summary>
        /// <param name="lotNo">The Lot No.</param>
        /// <param name="propertyNo">The Property No.</param>
        /// <returns></returns>
        public static NDbResult<List<Cord2ndTwistingNumber>> GetsByLotNo(string lotNo, int propertyNo)
        {
            MethodBase med = MethodBase.GetCurrentMethod();

            NDbResult<List<Cord2ndTwistingNumber>> ret = new NDbResult<List<Cord2ndTwistingNumber>>();
            ...
            try
            {
                ...
                if (null != items)
                {
                    foreach (var item in items)
                    {
                        ...
                        inst.PropertyNo = 8; // 2nd Twisting Number = 8
                        inst.SPNo = item.SPNo;
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static NDbResult<Cord2ndTwistingNumber> Save(Cord2ndTwistingNumber value)
        {
            MethodBase med = MethodBase.GetCurrentMethod();

            NDbResult<Cord2ndTwistingNumber> ret = new NDbResult<Cord2ndTwistingNumber>();
